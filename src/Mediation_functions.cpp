    ...
    for(int iter=0; iter<n_mcmc; iter++){
      ...
      for(arma::uword m=0; m < num_region; m++){
        ...
        time_seg(iter,m,2) = sys_t2;
          ...
          //   all_log_target_density(m) = log_target_density;
          // }else{
          //   log_target_density = all_log_target_density(m);
          // }
          ...
          // arma::colvec temp_new = Y_star-M_t.cols(delta_new)*(beta_new.rows(delta_new)-sign(beta_new.rows(delta_new))*lambda)/1.00;
      ...
      if( (iter%interval_step==0) & (iter>0)  ){
        ...
        if(iter<stop_burnin){
          // colvec sigma_t = sqrt(2*step_all);
          // for(uword l = 0; l<num_block; l++){
          //   sigma_t(l) = adjust_acceptance(emp_accept(iter/interval-1,l),sigma_t(l),target_accept);
      ...
      arma::uword back =  (n_mcmc - stop_burnin)/(n_mcmc/100) ;
